...
def draw_top13_chart(df, main_title, sub_title, is_ath=False):
    ...
    # [수정된 V8.8] 제목 겹침 원천 차단 (절대좌표 사용) & 폰트 크기 튜닝(16/12)
...
def draw_normal_chart(df, main_title, sub_title):
    ...
    # [수정된 V8.8] 제목 절대좌표 사용 & 폰트 크기 튜닝(16/12)
    fig.text(0.50, 0.98, main_title, ha='center', va='top', fontsize=16, fontweight='bold', color='black')
...
def draw_trend_chart(targets, base_date, period, close_df, custom_mapping, github_token):
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            range=[base_dt, global_max_x_date + timedelta(days=int(date_span * 0.15))], 
            showline=True, 
            linewidth=1.5, 
            linecolor='#CCCCCC', 
            mirror=True, 
...
def draw_deep_dive_chart(ticker_symbol, raw_df, ticker_name, plot_days):
    try:
        ...
        # [수정된 V8.8] 딥다이브 제목 포맷 및 계급 통일 (기존 빌트인 타이틀 삭제 후 annotation 사용)
        fig.update_layout(
            title=None,
            font=dict(family="Malgun Gothic, Arial"),
            height=750, 
...
# ==========================================
def main():
    st.set_page_config(page_title="HanMARI V8.8", layout="wide")
    ...
    st.markdown("<h3>📊 HanMARI V8.8</h3>", unsafe_allow_html=True)
