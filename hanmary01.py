...
SHARES_B = {
    ...
    'AVGO': 4.63, 'TSLA': 3.18, 'META': 2.54, 'BTC': 0.0196,
    'Samsung': 5.969
}
...
def draw_top13_chart(df, main_title, sub_title, is_mini=False, is_ath=False):
    ...
    # [수정] ax1 상단 차트 동적 Y축 스케일링 & 글씨 여백 뚫어주기
    ax1_mcaps = df[df['mcap'] > 10]['mcap'] 
    ax1_max = ax1_mcaps.max() if not ax1_mcaps.empty else 34.0
    ax1_upper_bound = int(np.ceil(ax1_max + 4.0)) # 막대 위 텍스트가 숨쉴 수 있게 4T 공간 확보
    ...
    for i, r in df.iterrows():
        ...
        if r['mcap'] > 10:
            # [수정] 하드코딩(34.5) 삭제 -> 막대 높이(r['mcap']) + 0.8T 여백에 글씨 배치
...
def generate_twitter_text(df, title, date_str, is_top=False):
    ...
    if is_top and 'display_rank' in df.columns:
        for _, r in df.iterrows():
            txt += f"{r['display_rank']}. {r['name']} {format_price(r['price'], r['name'])}\n"
