...
# ==========================================
# 0. Font & Global Settings (V5.0 대통합)
# ==========================================
@st.cache_resource
def font_setting():
    ...
    if system_name == 'Windows':
        ...
        plt.rc('font', family='NanumGothic') # 차트, 텍스트 모두 모던한 고딕체로 일치
...
# ==========================================
# 3. Chart Drawing (V5.0 Dynamic Precision & Y-axis limits)
# ==========================================
...
def style_axes(ax):
    ...
    # Y축 숫자가 다닥다닥 붙지 않도록 눈금 개수를 최대 4개로 강제 제한
...
def draw_normal_chart(df, main_title, sub_title):
    ...
    # Y축 동적 소수점 적용 (최대값에 따라 0.08% 도 구분 가능하도록)
...
def draw_deep_dive_chart(ticker_symbol, raw_df, ticker_name, plot_days):
    try:
        ...
        # 1. Y축 잠금으로 상하 스크롤 갇힘 방지 (fixedrange=True)
        fig.update_yaxes(fixedrange=True)
        
        # 2. X축 하단 영역 줌 컨트롤 바 부활 및 상단 원터치 버튼 추가
        fig.update_xaxes(
        ...
        # 타이틀 영문 고딕체(sans-serif) 완벽 일치 및 군더더기 제거
        fig.update_layout(
            title=dict(
