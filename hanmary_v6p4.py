...
def download_extra_data(tickers_tuple):
    if not tickers_tuple: return pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
...
# ==========================================
def draw_trend_chart(targets, base_date, period, close_df, custom_mapping):
    if not targets:
    ...
    # [수정] 카테고리별로 선 개수를 카운트하여 스타일을 다르게 배정
    category_counts = {}
    ...
    for name in targets:
        ...
        # [수정] 카테고리 고유 색상 및 선 스타일 교차 적용
        color = CATEGORY_COLORS.get(cat, '#777777')
        line_styles = ['solid', 'dash', 'dot', 'dashdot']
        style_idx = category_counts.get(cat, 0) % len(line_styles)
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='lines', 
            name=name, 
            line=dict(width=2, color=color, dash=current_style)
        ))
    ...
    fig.update_layout(
        ...
        hovermode="x unified",
        height=450,
        margin=dict(l=40, r=40, t=100, b=40), # [수정] 상단 여백 100으로 대폭 확대
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1, bgcolor='rgba(255,255,255,0.8)', bordercolor='#CCCCCC', borderwidth=1),
        xaxis=dict(
    ...
    # [수정] y좌표를 여백 안으로 당겨와 제목 잘림 원천 방지
    fig.add_annotation(
...
def main():
    st.set_page_config(page_title="HanMARI V6.2", layout="wide")
    ...
    with st.sidebar:
        ...
        if mode == "Market Overview":
            ...
            all_names_list = list(all_deep_dive_map.keys())
            trend_targets = st.multiselect("3) 비교 항목 (Compare Assets)", options=all_names_list, default=["BTC", "NASDAQ", "Gold"])
    ...
    st.markdown("<h3 style='font-size: 26px; font-weight: bold; font-family: sans-serif;'>📊 HanMARI V6.2 : Financial Secretary</h3>", unsafe_allow_html=True)
    ...
    if st.button('🚀 Run Analysis', use_container_width=True):
        ...
        if mode == "Deep Dive (Interactive)":
            ...
            draw_trend_chart(trend_targets, trend_base_date, trend_period, close_df, custom_mapping)
