...
# [3-1] 글로벌 12 전용 (Y축 1부터 시작 + 물결 간격 대폭 축소)
def draw_global_12_chart(df, main_title, sub_title, is_mini=False):
    if df.empty: return
    
    base_caps = {
        '금': 33.9, '엔비디아': 4.51, '은': 4.32, '애플': 4.08, 'MS': 2.98, 
        '아마존': 2.26, '구글': 1.88, 'TSMC': 1.81, '브로드컴': 1.58, '테슬라': 1.54, 
        '메타': 1.45, 'BTC': 1.37
    }
    ...
    # [수정] 고급 물결 무늬 (초슬림 & 위치 조정)
    wave_x = np.linspace(0, 1, 100)
    # 상단 박스 바닥 물결
    wave_y1 = np.sin(20 * np.pi * wave_x) * 0.008
    ax1.plot(wave_x, wave_y1, transform=ax1.transAxes, color='#CCCCCC', lw=0.8, clip_on=False)
    # 하단 박스 천장 물결
    wave_y2 = 1 + np.sin(20 * np.pi * wave_x) * 0.008
    ax2.plot(wave_x, wave_y2, transform=ax2.transAxes, color='#CCCCCC', lw=0.8, clip_on=False)
...
def main():
    ...
    mode_select = st.sidebar.radio("모드 선택", ["일반 분석", "종합 리포트"])
