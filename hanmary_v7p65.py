...
# ==========================================
# 4. New Trend Mode Chart (V7.65 완전 가독성 통제)
# ==========================================
...
def draw_trend_chart(targets, base_date, period, close_df, custom_mapping, github_token):
    ...
    # [팩트 1 보완] 차트 스케일 범위를 오직 데이터 기준으로만 강제 고정
    if global_max_y == float('-inf'):
        global_min_y, global_max_y = 0, 100
    y_margin = (global_max_y - global_min_y) * 0.05 if global_max_y != global_min_y else 5
    y_range = [global_min_y - y_margin, global_max_y + y_margin]

    # [팩트 2] 독립 분산 알고리즘 (픽셀 단위로만 글자 분산)
    end_points.sort(key=lambda x: x['y'], reverse=True)
    if end_points:
        chart_height_px = 350
        min_px_dist = 18 # 18픽셀의 최소 간격 보장
        
        for ep in end_points:
            norm_y = (ep['y'] - global_min_y) / (global_max_y - global_min_y + 1e-9)
            ep['px_y'] = norm_y * chart_height_px
            ep['target_px_y'] = ep['px_y']
            
        # Top-down pass (위에서 아래로 밀기)
        for i in range(1, len(end_points)):
            if end_points[i-1]['target_px_y'] - end_points[i]['target_px_y'] < min_px_dist:
                end_points[i]['target_px_y'] = end_points[i-1]['target_px_y'] - min_px_dist
                
        # Bottom-up pass (아래서 위로 보정)
        for i in range(len(end_points)-2, -1, -1):
            if end_points[i]['target_px_y'] - end_points[i+1]['target_px_y'] < min_px_dist:
                end_points[i]['target_px_y'] = end_points[i+1]['target_px_y'] + min_px_dist

        for ep in end_points:
            ay_offset = ep['px_y'] - ep['target_px_y'] # 양수면 아래로, 음수면 위로 이동
            ...
            fig.add_annotation(
                ...
                ax=40, ay=ay_offset, # x축으로 40px 우측 배치, y축은 픽셀 기반 절대 분산
    ...
    fig.update_layout(
        ...
        height=450,
        showlegend=False, 
        margin=dict(l=40, r=120, t=100, b=40), # 글자가 잘리지 않도록 우측 120px 방파제 확보
        xaxis=dict(
            showline=True, linewidth=1.5, linecolor='#CCCCCC', mirror=True,
            showgrid=False, ticks='outside'
        ),
        yaxis=dict(
            range=y_range, # ★ 주객전도 차단! 그래프를 Y축에 꽉 차게 절대 고정
            showline=True, linewidth=1.5, linecolor='#CCCCCC', mirror=True,
...
def main():
    st.set_page_config(page_title="HanMARI V7.65", layout="wide")
    ...
    with st.sidebar:
        ...
        github_token = st.text_input("GitHub Token (ghp_...)", type="password", value="")
        ...
        if github_token:
            ...
            st.info("💡 깃허브 토큰(비밀번호)을 입력하시면 실시간 업데이트 창이 활성화됩니다.")
    ...
    st.markdown("<h3 style='font-size: 26px; font-weight: bold; font-family: sans-serif;'>📊 HanMARI V7.65 : Financial Secretary</h3>", unsafe_allow_html=True)
