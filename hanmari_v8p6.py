...
# ==========================================
# 4. New Trend Mode Chart (V8.6 수평거리 반토막 / 밀집도 향상)
# ==========================================
...
def draw_trend_chart(targets, base_date, period, close_df, custom_mapping, github_token):
    ...
    if end_points:
        ...
        # [V8.6 보정 1] 수평 거리 8% -> 4% 반토막 축소
        target_x = global_max_x_date + timedelta(days=int(date_span * 0.04)) 
    ...
    fig.update_layout(
        ...
        height=450, 
        showlegend=False, 
        # [V8.6 보정 2] 거리를 줄인 만큼 우측 여백(방파제)을 220px에서 180px로 회수 (차트 본체 확장)
        margin=dict(l=40, r=180, t=100, b=40), 
        xaxis=dict(
            showline=True, 
            linewidth=1.5, 
            linecolor='#CCCCCC', 
...
# ==========================================
def main():
    st.set_page_config(page_title="HanMARI V8.6", layout="wide")
    ...
    st.markdown("<h3>📊 HanMARI V8.6</h3>", unsafe_allow_html=True)
