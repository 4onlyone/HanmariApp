...
# ==========================================
# 4. New Trend Mode Chart (Real Estate 융합)
# ==========================================
def draw_trend_chart(targets, base_date, period, close_df, custom_mapping, github_token):
    ...
    for name in targets:
        ...
        if ticker == "REAL_ESTATE":
            ...
            end_dt = pd.Timestamp.today().normalize() + pd.Timedelta(days=7)
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='lines', 
            name=name, 
            line=dict(width=line_width, color=color, dash=current_style)
        ))
    ...
    # [수정] 텍스트 박스를 복사 버튼이 내장된 st.code 블록으로 변경
    if summary_data:
...
def main():
    st.set_page_config(page_title="HanMARI V7.6", layout="wide")
    ...
    st.markdown("<h3 style='font-size: 26px; font-weight: bold; font-family: sans-serif;'>📊 HanMARI V7.6 : Financial Secretary</h3>", unsafe_allow_html=True)
