...
def font_setting():
    ...
    if system_name == 'Windows':
        ...
        plt.rc('font', family='NanumGothic') # 깔끔하고 모던한 고딕체로 전면 통일
...
def process_data(target_names, period, status_mode, close_df, high_df, custom_mapping=None):
    ...
    for name in target_names:
        ...
        elif name in ['TSMC']: cat = 'Others' # TSMC 예외 처리
...
def draw_deep_dive_chart(ticker_symbol, raw_df, ticker_name, plot_days):
    try:
        ...
        # 폰트를 앱 기본 폰트(Sans-serif)와 일치시킴
        fig.update_layout(
            title=dict(
                text=f"<b>{ticker_name} Technical Analysis</b>",
                x=0.5, xanchor='center', y=0.95, font=dict(size=26)
            ),
...
# ==========================================
def main():
    st.set_page_config(page_title="HanMARI V5.0", layout="wide")
    ...
    # 앱 타이틀에 기본 산세리프 폰트를 명시적으로 지정하여 일체감 형성
    st.markdown("<h3 style='font-size: 26px; font-weight: bold; font-family: sans-serif;'>📊 HanMARI V5.0 : Financial Secretary</h3>", unsafe_allow_html=True)
