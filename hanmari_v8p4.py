...
DEFAULT_PORTFOLIOS = {
    "Slot_A": {
        "name": "기본", 
        "tickers": "PLTR=Palantir, TSLA=Tesla, 102110.KS=Tiger 200, 139230.KS=Tiger Heavy, 411060.KS=Tiger Gold, BTC, ETH, XRP-USD=XRP, SOL-USD=SOL, KAIA-USD=KAIA, 133690.KS=TIGER QQQ"
    },
    "Slot_B": {
        "name": "나의 투자", 
        "tickers": "133690.KS=TIGER QQQ, 381170.KS=TIGER 테크TOP10, 139230.KS=TIGER 200 중공업, 411060.KS=TIGER KRX금현물, TSLA=Tesla, 102110.KS=TIGER 200, QQQ, PLTR=Palantir, 042370.KQ=비츠로테크, BTC, ETH"
    },
    "Slot_C": {
        "name": "국가별 Tiger ETF", 
...
def format_price(value, name, category='Others'):
    krw_assets = ['Samsung', 'SK Hynix', 'Tiger', 'TIGER', '테크TOP10', '중공업', '비츠로테크', '금현물']
...
def process_data(target_names, period, status_mode, close_df, high_df, open_df, custom_mapping=None):
    ...
    for name in target_names:
        ...
        # [V8.4 팩트] KOSPI, S&P500 등 명시적 카테고리 분류 100% 원복
        if name in ['Samsung', 'SK Hynix', 'KOSPI', 'TIGER 200'] or '.KS' in ticker or '.KQ' in ticker or '^KS' in ticker: 
            cat = 'K-Market'
...
def draw_normal_chart(df, main_title, sub_title):
    ...
    # [V8.4 팩트] 막대그래프 하이브리드 마진 적용 (10% 비율 + 최소 4.0 절대치 보장)
    pure_margin = max(abs(df['change'].max()), abs(df['change'].min()), 1) * 0.10 + 4.0
    ax.set_ylim(df['change'].min() - pure_margin, df['change'].max() + pure_margin) 
...
# ==========================================
# 4. New Trend Mode Chart (V8.4 렌더링 충돌 해결)
# ==========================================
...
def draw_trend_chart(targets, base_date, period, close_df, custom_mapping, github_token):
    ...
    for name in targets:
        ...
        if name == 'Seoul APT': 
            cat = 'Real Estate'
        elif name in ['Samsung', 'SK Hynix', 'KOSPI', 'TIGER 200'] or '.KS' in ticker or '.KQ' in ticker or '^KS' in ticker: 
            cat = 'K-Market'
        elif name in ['BTC', 'ETH'] or '-USD' in ticker: 
            cat = 'Crypto'
        elif name in ['Gold', 'Silver', 'Copper', 'Dollar Index', 'USD/KRW', 'USO', 'BNO'] or '=F' in ticker: 
    ...
    # [V8.4 팩트] 렌더링 충돌(ValueError)을 해결한 'Data 좌표 기반' 완벽 등분할
    end_points.sort(key=lambda x: x['y'], reverse=True)
    if end_points:
        n_items = len(end_points)
        
        date_span = (global_max_x_date - pd.to_datetime(base_dt)).days
        if date_span < 1: date_span = 1
        
        target_x = global_max_x_date + timedelta(days=int(date_span * 0.03)) 
        
        slot_max_y = global_max_y - (y_span * 0.05)
        slot_min_y = global_min_y + (y_span * 0.05)
        
        for i, ep in enumerate(end_points):
            if n_items > 1:
                target_y = slot_max_y - i * ((slot_max_y - slot_min_y) / (n_items - 1))
            else:
                target_y = (slot_max_y + slot_min_y) / 2
            ...
            fig.add_annotation(
                ...
                ax=target_x, 
                ay=target_y,  
                axref="x",     # 문법 에러(paper) 제거. 파이썬 공식 Data 좌표(x) 사용
                ayref="y",     # 파이썬 공식 Data 좌표(y) 사용
...
def draw_deep_dive_chart(ticker_symbol, raw_df, ticker_name, plot_days):
    try:
        ...
        fig.update_layout(
            title=dict(text=f"<b>{ticker_name} Technical Analysis</b>", x=0.5, font=dict(size=24)), 
            height=750, 
...
# ==========================================
def main():
    st.set_page_config(page_title="HanMARI V8.4", layout="wide")
    ...
    st.markdown("<h3>📊 HanMARI V8.4</h3>", unsafe_allow_html=True)
        ...
        if mode == "Deep Dive (Interactive)": 
            ...
            if show_global:
                ...
                if not df_g.empty: 
                    ...
                    st.code(generate_twitter_text(df_g, t_name, sub_t, True))
            ...
            if show_key:
                ...
                if not df_k.empty: 
                    ...
                    st.code(generate_twitter_text(df_k, "Key Indicators", sub_t))
            ...
            for p_data in active_slots.values():
                ...
                if not df_c.empty: 
                    ...
                    st.code(generate_twitter_text(df_c, p_data['name'], sub_t))
