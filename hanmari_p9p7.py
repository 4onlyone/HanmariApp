...
# [V9.7 수술] 글자 종류(한글,대문자,소문자/숫자)별로 실제 물리적 픽셀 두께(Bold 기준)를 정교하게 차등 계산하는 스캐너
def calculate_pixel_width(text):
    w = 0.0
    for c in text:
        if '\uac00' <= c <= '\ud7a3': # 한글 (뚱뚱함)
            w += 16.0
        elif '\u1100' <= c <= '\u11ff' or '\u3130' <= c <= '\u318f': # 한글 자음/모음
            w += 16.0
        elif c.isupper(): # 대문자
            w += 10.0 
        else: # 소문자, 숫자, 공백, 특수문자 (여백 확보를 위해 단가 상향)
            w += 8.5
    return w
...
def draw_trend_chart(targets, base_date, period, close_df, custom_mapping, github_token):
    ...
    if end_points:
        ...
        # [V9.7 수술] 한글/영문 차등 스캐너를 통해 가장 넓은 물리적 픽셀 폭 도출
        max_text_pixel_width = max([calculate_pixel_width(ep['name']) for ep in end_points]) if end_points else 85.0
        
        # 15px(지시선 화살표) + 뚱뚱함이 반영된 글자 길이 + 5px(순수 숨통 여백)
        required_pixels = 15.0 + max_text_pixel_width + 5.0
...
# ==========================================
def main():
    st.set_page_config(page_title="HanMARI V9.7", layout="wide")
    ...
    st.markdown("<h3>📊 HanMARI V9.7</h3>", unsafe_allow_html=True)
