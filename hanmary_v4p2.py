...
def font_setting():
    ...
    if system_name == 'Windows':
        ...
        plt.rc('font', family='NanumMyeongjo') # 묵직하고 고급스러운 명조체 적용
...
def process_data(target_names, period, status_mode, close_df, high_df, custom_mapping=None):
    ...
    for name in target_names:
        ...
        elif name in ['Gold', 'Silver', 'Copper', 'Dollar Index', 'USD/KRW']: cat = 'Macro'
...
# ==========================================
# 3. Chart Drawing (Dynamic Format)
# ==========================================
def get_pct_str(val):
    # 변화량 소수점 절대 규칙: 0은 0%, 100이상이면 소수점 버림, 나머지는 무조건 1자리
    if abs(val) < 0.005: return "0%"
    dec = 0 if abs(val) >= 100 else 1
    sign = '▲' if val > 0 else '▼'
    return f"{sign}{abs(val):.{dec}f}%"

def format_pct_text(val):
    if abs(val) < 0.005: return "0%"
    dec = 0 if abs(val) >= 100 else 1
    sign = '+' if val > 0 else '-'
    return f"{sign}{abs(val):.{dec}f}%"
...
def draw_normal_chart(df, main_title, sub_title):
    ...
    for bar in bars:
        ...
        pct_str = get_pct_str(h)
        ax.annotate(pct_str, xy=(bar.get_x() + bar.get_width()/2, h), xytext=(0, offset), textcoords="offset points", ha='center', va=va, fontweight='bold', fontsize=9, color=txt_col)
...
def generate_twitter_text(df, title, date_str, is_top=False):
    ...
    if is_top and 'display_rank' in df.columns:
        for _, r in df.iterrows():
            ...
            change_str = format_pct_text(r['change'])
...
# ==========================================
def main():
    st.set_page_config(page_title="HanMARI V4.2", layout="wide")
    ...
    st.markdown("<h3 style='font-size: 26px; font-weight: bold;'>📊 HanMARI V4.2 : Financial Secretary</h3>", unsafe_allow_html=True)
