...
def draw_trend_chart(targets, base_date, period, close_df, custom_mapping):
    ...
    fig.update_layout(
        ...
        # [수정] 범례 위치를 도화지 안쪽 좌측 하단(데드 스페이스)으로 이동
        legend=dict(
            orientation="v", 
...
def main():
    st.set_page_config(page_title="HanMARI V6.3", layout="wide")
    ...
    with st.sidebar:
        ...
        if mode == "Market Overview":
            ...
            # [수정] 종목들을 카테고리별로 자동 분류하여 폴더 생성
            cat_groups = {'US Tech': [], 'K-Market': [], 'Crypto': [], 'Macro': [], 'Others': []}
            ...
            default_trends = ["BTC", "NASDAQ", "Gold"]
            
            # [수정] 좁은 multiselect 대신 모바일 친화적인 폴더(Expander) 내부 체크박스 적용
    ...
    st.markdown("<h3 style='font-size: 26px; font-weight: bold; font-family: sans-serif;'>📊 HanMARI V6.3 : Financial Secretary</h3>", unsafe_allow_html=True)
