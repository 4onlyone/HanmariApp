...
DEFAULT_PORTFOLIOS = {
    ...
    "Slot_B": {"name": "한국/ETF (B)", "tickers": "005930.KS=Samsung Elec, 005380.KS=Hyundai Motors, 006800.KS=Mirae Asset, 139260.KS=Tiger Heavy, 438900.KS=HANARO K-Food, 360750.KS=Tiger S&P500, 102110.KS=Tiger 200, 139220.KS=Tiger Construction, 453850.KS=Tiger India, 411060.KS=Tiger Gold"},
...
TICKERS = {
    ...
    'KOSPI': '^KS11', 'NASDAQ': '^IXIC', 'S&P 500': '^GSPC', 'DXY': 'DX-Y.NYB', 'USD/KRW': 'KRW=X',
    ...
    'TIGER 200': '102110.KS', 'TIGER Heavy': '139260.KS', 'TIGER Nasdaq': '133690.KS', 
...
# ==========================================
# 2. Data Engine (Enhanced Timezone Logic)
# ==========================================
...
def process_data(target_names, period, status_mode, close_df, high_df, custom_mapping=None):
    ...
    for name in target_names:
        ...
        cat = 'K-Market' if ('.KS' in ticker or '.KQ' in ticker or name in ['Samsung', 'SK Hynix', 'KOSPI']) else \
              'Crypto' if ('-USD' in ticker or name in ['BTC', 'ETH']) else \
              'Macro' if name in ['Gold', 'Silver', 'Copper', 'DXY', 'USD/KRW'] else \
              'US Tech' if ticker.isalpha() and ticker.isupper() else 'Others'
...
# ==========================================
# 3. Chart Drawing (V3.0 Matplotlib Format)
# ==========================================
def style_axes(ax):
...
def draw_top13_chart(df, main_title, sub_title, is_ath=False):
    ...
    for i, r in df.iterrows():
        ...
        (ax1 if r['mcap'] > 10 else ax2).text(i, r['mcap'] + (0.8 if r['mcap'] > 10 else 0.3), lbl, ha='center', va='bottom', fontsize=9, fontweight='bold', color=txt_col)
...
def draw_normal_chart(df, main_title, sub_title):
    ...
    for bar in bars:
        ...
        ax.annotate(f"{sign}{abs(h):.1f}%", xy=(bar.get_x() + bar.get_width()/2, h), xytext=(0, offset), textcoords="offset points", ha='center', va=va, fontweight='bold', fontsize=9, color=txt_col)
...
def draw_deep_dive_chart(ticker_symbol, raw_df, ticker_name, plot_days):
    try:
        ...
        # 1. 겹침 방지를 위해 margin_t(상단 여백) 120으로 2배 확대
        # 2. 제목은 고풍스러운 영문 세리프체(Times New Roman) 강제 적용
        # 3. 범례(Legend) 위치를 차트 내부가 아닌 상단 빈 공간으로 분리
        fig.update_layout(
            title=dict(
                text=f"<span style=\"font-family: 'Times New Roman', Times, serif; font-weight: bold; font-size: 26px;\">{ticker_name} Technical Analysis</span>",
                x=0.5, xanchor='center', y=0.95
            ),
            height=750, margin=dict(l=10, r=10, t=120, b=10), 
...
# ==========================================
def main():
    st.set_page_config(page_title="HanMARI V4.0", layout="wide")
    ...
    st.markdown("<h3 style='font-size: 26px; font-weight: bold;'>📊 HanMARI V4.0 : Financial Secretary</h3>", unsafe_allow_html=True)
        ...
        if mode == "Deep Dive (Interactive)":
            ...
            if show_global:
                ...
                if not df_g.empty:
                    ...
                    sub_t = "All-Time High" if status == 'ATH' else f"{get_korea_time().strftime('%b %d %H:%M KST') if status != 'Completed' else df_g['curr_date'].max().strftime('%b %d')} vs {df_g['base_date'].max().strftime('%b %d')}"
            ...
            if show_key:
                df_k = process_data(['Gold','Silver','Copper','BTC','ETH','KOSPI','NASDAQ','S&P 500','DXY','USD/KRW'], period, status, close_df, high_df)
                if not df_k.empty:
                    df_k = sort_by_category(df_k)
                    sub_t = "All-Time High" if status == 'ATH' else f"{get_korea_time().strftime('%b %d %H:%M KST') if status != 'Completed' else df_k['curr_date'].max().strftime('%b %d')} vs {df_k['base_date'].max().strftime('%b %d')}"
            ...
            for k, p_data in active_slots.items():
                ...
                if not df_c.empty:
                    ...
                    sub_t = "All-Time High" if status == 'ATH' else f"{get_korea_time().strftime('%b %d %H:%M KST') if status != 'Completed' else df_c['curr_date'].max().strftime('%b %d')} vs {df_c['base_date'].max().strftime('%b %d')}"
