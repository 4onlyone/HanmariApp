...
def draw_normal_chart(df, main_title, sub_title):
    ...
    # [V9.3 수술] 모니터 차트의 실사용 높이(약 280px)를 역산하여, 정확히 40px 절대공간 확보
...
def draw_trend_chart(targets, base_date, period, close_df, custom_mapping, github_token):
    ...
    if end_points:
        ...
        # [V9.3 수술] 글자 시작점(ax)을 개별 종목 날짜가 아닌 "최근 데이터 일괄 날짜"로 고정하여 좌측 칼각 정렬 복구
        ...
        for i, ep in enumerate(end_points):
            ...
            fig.add_annotation(
                ...
                ax=text_x_pos, # 모두 이 단일 수직선에 일치시킵니다.
                ay=target_y,  
                axref="x",     
                ayref="y",     
...
def draw_deep_dive_chart(ticker_symbol, raw_df, ticker_name, plot_days):
    try:
        ...
        # [V9.3 수술] 캔들의 두께(절반=0.5일) + 순수 5픽셀을 합쳐서 절대 여백 확보
        assumed_plot_width = 800.0
...
# ==========================================
def main():
    st.set_page_config(page_title="HanMARI V9.3", layout="wide")
    ...
    st.markdown("<h3>📊 HanMARI V9.3</h3>", unsafe_allow_html=True)
