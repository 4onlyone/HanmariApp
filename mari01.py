    ...
    if status_mode == 'ATH':
        ...
        for name in target_list:
            ...
            try:
                ...
                ath_price = float(hist['High'].max().item())
        ...
        for name in target_list:
            ...
            try:
                ...
                if '일간' in period_type and status_mode != 'Cycle':
                    if status_mode == 'Completed':
                        d_date = today - timedelta(days=2)
                    else:
                        d_date = today - timedelta(days=1)
                    base = get_price_at_date(ticker, d_date)
                else:
...
def main():
    ...
    # 기간 선택 (종합 리포트 모드이거나 ATH 모드면 기간 선택 불필요)
    if mode == "종합 리포트" or status == 'ATH 대비':
        period = '전체' # 로직상 무시됨
        engine_status = 'ATH' if status == 'ATH 대비' else 'Cycle' # 종합리포트는 보통 Cycle로 봄 (또는 Live)
        # 종합 리포트일 때도 기준(Live/마감/주기)은 적용되어야 함.
        ...
        if mode == "일반 분석":
            for t_name, t_list, t_func in targets:
                ...
                with st.spinner(f'{t_name} 분석 중...'):
                    ...
                    if not df.empty:
                        base_d = df['base_date'].iloc[0] if engine_status != 'ATH' else "ATH"
                        base_str = f"({base_d} 기준)" if engine_status != 'ATH' else "(역대 최고가 대비)"
                    ...
                    for i, p in enumerate(periods):
                        with cols_map[i]: 
                            with st.spinner(f'{p}...'):
                                ...
                                if not df.empty:
                                    base_d = df['base_date'].iloc[0]
                                    prefix = f"[{curr_full_str}]" if p == '일간' else f"[{curr_date_str}]"
                                    # 미니 차트 그리기
                                    if t_name == "글로벌 Top 12":
                                        t_func(df, f"{prefix} {p}", f"({base_d} 기준)", is_mini=True)
                                    else:
                                        t_func(df, f"{prefix} {p}", f"({base_d} 기준)", is_mini=True)
