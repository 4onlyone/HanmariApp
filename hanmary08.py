...
def process_data(target_names, period, status_mode, close_df, high_df, custom_mapping=None):
    ...
    for name in target_names:
        ...
        if status_mode == 'ATH':
            ...
            if status_mode == 'Completed': valid_series = series[series.index.date < today_kst]
            else: valid_series = series 
        ...
        mcap = 0
        if name in SHARES_B:
            if name == 'Samsung': mcap = (curr * SHARES_B[name]) / usd_krw_current / 1000
            else: mcap = (curr * SHARES_B[name]) / 1000
...
def sort_by_category(df):
    ...
    # 카테고리로 1차 정렬 후, 이름(알파벳순)으로 2차 정렬 적용
...
def main():
    ...
    if show_custom:
        ...
        custom_input = st.sidebar.text_input("Add Tickers (Separate with commas ',')", placeholder="e.g., 035720.KS=Kakao, AAPL")
