...
# ==========================================
# 4. New Trend Mode Chart (V7.63 가독성 향상)
# ==========================================
...
def draw_trend_chart(targets, base_date, period, close_df, custom_mapping, github_token):
    ...
    # [수정] 지시선 자동 분산 알고리즘 (닻 고정 및 정확한 밀어내기)
    end_points.sort(key=lambda x: x['y'], reverse=True)
    if end_points:
        max_y = end_points[0]['y']
        min_y = end_points[-1]['y']
        threshold = max(2.0, (max_y - min_y) * 0.05) 
        
        shifts = [0] * len(end_points)
        current_shift = 0
        
        for i in range(1, len(end_points)):
            effective_prev_y = end_points[i-1]['y'] - (current_shift / 20.0) * threshold
            effective_diff = effective_prev_y - end_points[i]['y']
            
            if effective_diff < threshold:
                current_shift += 20 # 20px씩 누적해서 아래로 강제 이동
            else:
                current_shift = 0 
        ...
        for i, ep in enumerate(end_points):
            ...
            fig.add_annotation(
                x=ep['x'], y=ep['y'],
                xref="x", yref="y",      # ★데이터 실제 위치에 닻 고정★
                axref="pixel", ayref="pixel", # 픽셀 단위로만 밀어내기
                text=f"<b>{short_name}</b>",
                showarrow=True,
                arrowcolor="rgba(150, 150, 150, 0.5)",
                arrowsize=1, arrowwidth=1, arrowhead=0,
                ax=35, ay=shifts[i],     # 겹침 방지를 위해 계산된 shift 값 적용
...
def main():
    st.set_page_config(page_title="HanMARI V7.63", layout="wide")
    ...
    st.markdown("<h3 style='font-size: 26px; font-weight: bold; font-family: sans-serif;'>📊 HanMARI V7.63 : Financial Secretary</h3>", unsafe_allow_html=True)
