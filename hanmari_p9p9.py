...
TICKERS = {
    ...
    'SpaceX': 'SPCX', 'LLY': 'LLY', 'BRK-B': 'BRK-B', 'MU': 'MU', # [V9.9] 마이크론(MU) 편입
...
SHARES_B = {
    ...
    'SpaceX': 13.06, 'LLY': 0.95, 'BRK-B': 2.15, 'MU': 1.11 # [V9.9] 삼성, 하이닉스 우선주/보통주 검증 완료
}
...
def fetch_github_real_estate(token):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            st.error(f"🚨 [GitHub API Error] 토큰이 만료되었거나 접근 권한이 없습니다. (Status: {res.status_code})")
            return None
    except Exception as e:
        st.error(f"🚨 [Data Fetch Error] 데이터를 불러오는 중 오류가 발생했습니다: {e}")
        return None
    return None
...
def update_github_real_estate(token, new_date, new_index):
    ...
    try:
        ...
        st.error(f"🚨 [Update Error] 깃허브 기록 중 문제 발생: {e}")
    return False
...
def process_data(target_names, period, status_mode, close_df, high_df, open_df, custom_mapping=None):
    ...
    for name in target_names:
        ...
        if name in ['Samsung', 'SK Hynix', 'KOSPI', 'TIGER 200', 'HLB', 'HL만도'] or '.KS' in ticker or '.KQ' in ticker or '^KS' in ticker: 
            ...
            cat = 'Others'
        elif name in ['NASDAQ', 'S&P 500', 'QQQ', 'NVDA', 'AAPL', 'MSFT', 'AMZN', 'GOOG', 'AVGO', 'TSLA', 'META', 'PLTR', 'SpaceX', 'LLY', 'BRK-B', 'MU'] or '^GSPC' in ticker or '^IXIC' in ticker or (ticker.isalpha() and ticker.isupper()): 
            cat = 'US Tech'
        else: 
            cat = 'Others'
...
# [V9.9 수술] BTC, 삼성전자, SK하이닉스 3대장 강제 생존 보장 로직
def format_top13_df(df, t_name):
    ...
    # 필수 생존 라인업: BTC, Samsung, SK Hynix
    mandatory_assets = ['BTC', 'Samsung', 'SK Hynix']
    
    for mandatory in mandatory_assets:
        matching_names = [n for n in df['name'].values if mandatory in n]
        if matching_names:
            m_name = matching_names[0]
            if m_name not in top_12['name'].values:
                missing_mandatory.append(df[df['name'] == m_name].iloc[0:1])
...
def draw_top13_chart(df, main_title, sub_title, is_ath=False):
    ...
    # [V9.9] 띄어쓰기를 감지하여 자동으로 줄바꿈 처리 (긴 이름 압축 방어)
    ...
    for i, r in df.iterrows():
        ...
        (ax1 if r['mcap'] > 10 else ax2).text(i, r['mcap'] + (0.3 if r['mcap'] > 10 else 0.1), lbl, ha='center', va='bottom', fontsize=8, fontweight='bold', color=txt_col)
...
def draw_trend_chart(targets, base_date, period, close_df, custom_mapping, github_token):
    ...
    if end_points:
        ...
        required_pixels = 10.0 + max_text_pixel_width + 5.0 
...
# ==========================================
def main():
    st.set_page_config(page_title="HanMARI V9.9", layout="wide")
    ...
    st.markdown("<h3>📊 HanMARI V9.9</h3>", unsafe_allow_html=True)
    ...
    if st.button('🚀 Run Analysis', use_container_width=True):
        ...
        if mode == "Deep Dive (Interactive)": 
            ...
            if show_global:
                # [V9.9] SK하이닉스 및 마이크론(MU) 글로벌 타겟 투입
                g_targets = ['Gold','NVDA','Silver','AAPL','MSFT','AMZN','GOOG','TSMC','AVGO','TSLA','META','BTC','SpaceX','LLY','BRK-B','MU','SK Hynix','Samsung']
                df_g = process_data(g_targets, period, status, close_df, high_df, open_df)
                if not df_g.empty: 
                    df_g, t_name = format_top13_df(df_g, "Global Top 12+α")
